...
const ClaimDetails = () => {
  ...
  const generateLogicalReasoning = (aiReasoning, step) => {
    if (aiReasoning === 'True') {
      return 'The conditions required for this step have been met. For example, the claim number may start with a required prefix, or all conditions set by the system are satisfied.';
    } 
    if (aiReasoning === 'False') {
      return `The conditions for this step have not been met. For example, the claim number does not match the required pattern, or necessary prior claims are missing.`;
    }
  
    // Add other conditions as necessary based on the AI reasoning
    if (aiReasoning.includes('Claim number')) {
      return `The claim number ${step.StepDescription || 'is invalid'} based on the AI criteria. For example, it might not start with the expected prefix.`;
    }
    
    if (aiReasoning.includes('History Claims')) {
      return `There are no history claims to evaluate for this step, which means the criteria for further evaluation are not met.`;
    }
  
    return 'The AI has determined that the current conditions do not meet the expected criteria. Please check the details for further information.';
  };
  return (
          ...
          {claimDetails.Steps?.length ? (
            <div className="log-table">
              <div className="log-table-header">
                ...
                <div className="log-cell reasoning-cell">Logical Reasoning</div>
              ...
              {claimDetails.Steps
                ...
                .map((step, index) => (
                  ...
                  >
                    ...
                    <div className="log-cell step-cell">{index + 1}</div>
                    ...
                    <div className="log-cell description-cell">
                      {step.KnowledgeSource || 'No Knowledge Source available.'}
                    </div>
